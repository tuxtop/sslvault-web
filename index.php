...
else
{
	...
	if ($form = new PostData() and $form->_post)
	{
		if ($form->username and $form->password)
		{
			...
			if ($query = $dbh->query("SELECT uid,username,admin FROM users WHERE UPPER(username)='$form->username' AND password='$form->password' AND enabled=true"))
			{
	...
	if ($error) $error = '<div class="alert alert-critical">'.$error.'</div>';
	print <<<FORM
